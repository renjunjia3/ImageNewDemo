...
//                mPorterDuffXferMode = getModeFromInteger(a.getInteger(R.styleable.MaskableLayout_porterduffxfermode, 0));
...
//    }
    ...
    public void setPath(Path path, int width, int height) {
        this.path = path;
        mFinalMask = getBitmap(width, height);
        invalidate();
    }

    public Path setPath(List<Point> pathPoints, int path_width) {
    ...
    public void setMaskBitmap(Bitmap mBitmap, Path path) {
        this.path = path;
        mFinalMask = mBitmap;
        invalidate();
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mPaint != null && mFinalMask != null) {
            mPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_OUT));//mPorterDuffXferMode
